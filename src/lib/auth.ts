  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // One-time use — clear all codes for this email
        await prisma.verificationToken.deleteMany({ where: { identifier: email } })

        // Find or create user (adapter doesn't fire for CredentialsProvider)
        let user = await prisma.user.findUnique({ where: { email } })
        if (!user) {
          user = await prisma.user.create({
            data: { email, emailVerified: new Date() },
          })
          // Initial credit grant (mirrors the createUser event for Google)
          await prisma.creditTransaction.create({
            data: {
              ...
              balance: 1500,
              type: 'initial_grant',
              metadata: { reason: 'signup_bonus_code' },
            },
          }).catch((e) => console.error('[auth] initial grant failed:', e))
        ...
        return { id: user.id, email: user.email ?? undefined, name: user.name, image: user.image }
      },
    }),
  ],
  session: {
