...
export default function CitationFormatPicker({ projectId, initialFormat, onChanged }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="bg-[#FAF7F0] border border-[#d4c9b5] rounded-sm p-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:items-center mb-2">
            ...
            {suggestion && selected !== suggestion.format && (
              <button
                type="button"
        ...
        <div className="bg-[#FAF7F0] border border-[#d4c9b5] rounded-sm p-4 flex items-center justify-between gap-3">
          ...
          {isDirty && (
            <span className="shrink-0 font-ui text-[10px] uppercase tracking-wider px-2 py-1 rounded-sm bg-amber-100 text-amber-900 font-semibold">
              Değişiklik var
            </span>
          )}
        </div>
      ...
      {/* MAIN — Format cards (left, 3-col) + Live preview (right, sticky) */}
      ...
      <section className="lg:col-span-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {ORDER.map((fmt) => {
            ...
            const visibleFields = meta.fields.slice(0, 2);
            const extraFields = meta.fields.length - visibleFields.length;
            return (
              <button
                key={fmt}
                type="button"
