...
export default function StyleChat({
    ...
    async (targetSessionId?: string) => {
      try {
        const url = targetSessionId
          ? `/api/style-profiles/${profileId}/chat/history?sessionId=${targetSessionId}`
          : `/api/style-profiles/${profileId}/chat/history`;
  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch(`/api/style-profiles/${profileId}/chat`, {
        method: "POST",
      ...
      if (res.status === 402) {
        ...
        toast.error(
          `Insufficient credits (${errData.balance ?? 0} remaining). You need ~${errData.cost ?? "?"} credits.`
        );
        setMessages(newMessages);
      ...
      while (true) {
        ...
        for (const line of parts) {
          ...
          try {
            ...
            if (parsed.step === "profile_updated") {
              setStreamingStep("Profile updated!");
            }

            if (parsed.chunk) {
              fullContent += parsed.chunk;
              const displayContent = stripStyleTags(fullContent);

              if (
                fullContent.includes("<style_profile>") &&
                !fullContent.includes("</style_profile>")
              ) {
                setStreamingStep("Building your style profile...");
              }
            ...
            if (parsed.done) {
              setMessages((prev) => {
                ...
                updated[assistantIndex] = {
                  ...updated[assistantIndex],
                  content: stripStyleTags(fullContent),
                };
                return updated;
              });

              if (parsed.profileUpdated) {
                // Fetch the updated profile
                try {
                  const profileRes = await fetch(
                    `/api/style-profiles/${profileId}`
                  );
                  if (profileRes.ok) {
      ...
      try {
        const histRes = await fetch(
          `/api/style-profiles/${profileId}/chat/history?sessionId=${sessionIdRef.current}`
        );
        if (histRes.ok) {
        ...
        <div className="flex items-center gap-1">
          {!isStreaming && (
            <Button
              ...
              className={`h-7 font-ui text-xs gap-1 ${
                showSessions
          ...
          {!isLoadingHistory && messages.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="font-body text-xs mt-1 opacity-70">
                I&apos;ll ask questions to build your Writing Twin profile.
              </p>
            </div>
          )}
          {messages.map((msg, i) => (
      ...
      <div className="border-t px-4 py-3 shrink-0">
        <div className="flex gap-2 items-end">
          <Textarea
            ...
            placeholder="Describe your writing style or paste a sample..."
