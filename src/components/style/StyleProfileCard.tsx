...
export default function StyleProfileCard({
  ...
  return (
    <div className="border border-sandy/60 rounded-sm bg-page/80 hover:border-gold/40 transition-all group">
      <button
        type="button"
      ...
      >
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <span className="shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded-sm bg-ink/5 font-ui text-[10px] text-ink-light">
            {profile.method === "chat" ? (
        ...
        {hasProfile && data ? (
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {data.sentenceLength && (
              <span className="px-2 py-0.5 rounded-sm bg-gold/10 font-ui text-[10px] text-ink-light">
        ...
        <p className="font-ui text-[10px] text-ink-muted">{date}</p>
      </button>

      <div className="border-t border-sandy/40 px-4 py-2 flex justify-end">
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
          ...
          className="flex items-center gap-1 font-ui text-[10px] text-ink-muted hover:text-red-600 transition-colors"
        >
